...
class ControllerExtensionPaymentGate2play extends Controller
{
    ...
    public function index()
    {
        ...
        $accessToken = $this->config->get('gate2play_access_token');
        $type = $this->config->get('gate2play_trans_type');
        $connector = $this->config->get('gate2play_connector');
        ...
        $ip = $order_info['ip'];
        $tokenization = $customer_id > 0 ? $this->config->get('gate2play_tokenization_status') : 0;
        if (empty($state)) {
            $state = $city;
        }
        ...
        $data['postbackURL'] = $url . 'index.php?route=extension/payment/gate2play/callback';

        return $this->load->view('extension/payment/gate2play.tpl', $data);
    }
    ...
    public function callback()
    {
        if (isset($_GET['id'])) {
            ...
            $url .= "?entityId=" . $this->config->get('gate2play_channel');
            ...
            if ($order_info) {
                ...
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                    // Order is not approved.
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_order_status_failed_id'), '', TRUE);
                    ...
                    $this->response->redirect($this->url->link('extension/payment/gate2play/fail', '', 'SSL'));
                }
                exit;
            } else {
    ...
    public function fail()
    {
        ...
        $this->response->setOutput($this->load->view('extension/payment/gate2play_fail.tpl', $data));
    }
