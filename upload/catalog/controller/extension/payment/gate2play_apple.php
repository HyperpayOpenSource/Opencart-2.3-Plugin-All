...
class ControllerExtensionPaymentGate2playApple extends Controller
{
    ...
    {
        $this->language->load('payment/gate2play_apple');
        $this->load->model('checkout/order');

        $data['button_confirm_applepay'] = $this->language->get('button_confirm');
        //--------------------------------------
        $testMode = $this->config->get('gate2play_apple_testmode');
        if ($testMode == 0) {
        ...
        $channel = $this->config->get('gate2play_apple_channel');
        $mode = $this->config->get('gate2play_apple_trans_mode');
        $accessToken = $this->config->get('gate2play_apple_access_token');
        $type = $this->config->get('gate2play_apple_trans_type');
        $connector = $this->config->get('gate2play_apple_connector');
        $amount = number_format(round($orderAmount, 2), 2, '.', '');
        $currency = $this->config->get('gate2play_apple_base_currency');
        ...
        $payment_brands = implode(' ', $this->config->get('gate2play_apple_brands'));
        //--------------------------------------
        $data['token_applepay'] = $token;
        ...
        $data['postbackURL_applepay'] = $url . 'index.php?route=extension/payment/gate2play_apple/callback';

        return $this->load->view('extension/payment/gate2play_apple.tpl', $data);
    }
    ...
    public function callback()
    {
        if (isset($_GET['id'])) {
            ...
            $url .= "?entityId=" . $this->config->get('gate2play_apple_channel');
            ...
            if ($order_info) {
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_apple_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                    // Order is not approved.
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_apple_order_status_failed_id'), '', TRUE);
                    $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg . Order Id: $orderid");
                    $this->session->data['gate2play_apple_error'] = $failed_msg;
                    $this->response->redirect($this->url->link('extension/payment/gate2play_apple/fail', '', 'SSL'));
                }
                exit;
            } else {
                if ($this->config->get('gate2play_apple_mailerrors') == 1) {
                ...
                //$this->model_checkout_order->confirm($orderid, $this->config->get('gate2play_apple_order_status_failed_id'), '', TRUE);
                $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_apple_order_status_failed_id'), '', TRUE);
                $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg. Order Id: $orderid");
                $this->response->redirect($this->url->link('extension/payment/gate2play_apple/fail', '', 'SSL'));
                print 'fff';
    ...
    public function fail()
    {
        ...
        if (isset($this->session->data['gate2play_apple_error'])) {
            $data['general_error'] = $this->session->data['gate2play_apple_error'];
        } else {
        ...
        $this->response->setOutput($this->load->view('extension/payment/gate2play_apple_apple_fail.tpl', $data));
    }
