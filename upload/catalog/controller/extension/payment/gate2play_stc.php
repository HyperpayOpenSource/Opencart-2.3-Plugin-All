...
class ControllerExtensionPaymentGate2playStc extends Controller
{
    ...
    {
        $this->language->load('payment/gate2play_stc');
        $this->load->model('checkout/order');

        $data['button_confirm'] = $this->language->get('button_confirm');
        //--------------------------------------
        $testMode = $this->config->get('gate2play_stc_testmode');
        if ($testMode == 0) {
        ...
        $channel = $this->config->get('gate2play_stc_channel');
        $mode = $this->config->get('gate2play_stc_trans_mode');
        $accessToken = $this->config->get('gate2play_stc_access_token');
        $type = $this->config->get('gate2play_stc_trans_type');
        $connector = $this->config->get('gate2play_stc_connector');
        $amount = number_format(round($orderAmount, 2), 2, '.', '');
        $currency = $this->config->get('gate2play_stc_base_currency');
        ...
        $payment_brands = implode(' ', $this->config->get('gate2play_stc_brands'));
        //--------------------------------------
        $data['token'] = $token;
        $data['payment_brands'] = $payment_brands;
        $data['scriptURL'] = $scriptURL . $token;

        $data['formStyle'] = $this->config->get('gate2play_stc_payment_style');
        ...
        $data['postbackURL'] = $url . 'index.php?route=extension/payment/gate2play_stc/callback';

        return $this->load->view('extension/payment/gate2play_stc.tpl', $data);
    }
    ...
    public function callback()
    {
        if (isset($_GET['id'])) {
            ...
            $url .= "?entityId=" . $this->config->get('gate2play_stc_channel');
            ...
            if ($order_info) {
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_stc_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                    // Order is not approved.
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_stc_order_status_failed_id'), '', TRUE);
                    $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg . Order Id: $orderid");
                    $this->session->data['gate2play_stc_error'] = $failed_msg;
                    $this->response->redirect($this->url->link('extension/payment/gate2play_stc/fail', '', 'SSL'));
                }
                exit;
            } else {
                if ($this->config->get('gate2play_stc_mailerrors') == 1) {
                ...
                //$this->model_checkout_order->confirm($orderid, $this->config->get('gate2play_stc_order_status_failed_id'), '', TRUE);
                $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('gate2play_stc_order_status_failed_id'), '', TRUE);
                $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg. Order Id: $orderid");
                $this->response->redirect($this->url->link('extension/payment/gate2play_stc/fail', '', 'SSL'));
                print 'fff';
    ...
    public function fail()
    {
        ...
        if (isset($this->session->data['gate2play_stc_error'])) {
            $data['general_error'] = $this->session->data['gate2play_stc_error'];
        } else {
        ...
        $this->response->setOutput($this->load->view('extension/payment/gate2play_stc_fail.tpl', $data));
    }
